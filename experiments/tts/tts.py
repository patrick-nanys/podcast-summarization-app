...
def speecht5_tts(
    input_text, embedding_type, custom_embedding_path=None, embedding_tensor_path=None, output_path="tts_example.wav"
):
...
def elevenlabs_tts(chunks, API_KEY, voice_name, output_path):
    ...
    )  # ElevenLabs always returns mp3 bytes. Maybe it should be converted to wav with ffmpeg?
...
if __name__ == "__main__":
    ...
    embedding_path_list = [
        os.path.join("downloaded_voices", file) for file in os.listdir("downloaded_voices") if file.endswith("wav")
    ][:60]
    ...
    speecht5_tts(
        ...
        custom_embedding_path=embedding_path_list,
        embedding_tensor_path="karoly_60.txt",
        output_path="tts_example_60.wav",
    )
